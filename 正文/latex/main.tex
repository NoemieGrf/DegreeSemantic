...
\ref{ref7}\ref{ref7a} shows the structure in which 

in which DegP selects QP and Q selects AP. 



If there is no difference between all degree words, they would be expected to show a symmetric behaviour but actually not. In Dutch, when a sentence uses the quantifier-like degree expression, the entire adjective phrase can be replaced by a pro-form. Base on this, Corver gives the structures of these two forms which are shown in \ref{ref7}.

In \ref{ref7}\ref{ref7a}, syntactic structure shows that the DegP head selects a QP and QP's head in turn selects AP. \ref{ref7}\ref{ref7b} shows that adjective phrases are introuduced by quantifier-like elements and the DegP is missing. The author tried to merge two structures into a single elegant structure. To make this happen, some evidence must be discovered to support the so-called "A to G raising" which is language specific. In Dutch, there are serval phenomena suggesting the existence of this movement. Take one example as \textit{genoeg}-inversion, which is illustrated in \ref{genoeg}\ref{genoeg_b}, and the structure of \textit{genoeg}-inversion is shown in \ref{genoeg}\ref{genoeg_c}.
...
\item \label{genoeg_b} Jan is [bang genoeg daarvoor]
...
Svenonius and Kennedy studied Null Degree Questions in Scandinavian\cite{svenonius2006}. The so-called Null Degree Question is such a phenomenon that, in some dialects in Scandinavian, a degree sentence can only have degree operator but no wh-word. For example, in \ref{ref8}, the first sentence is Icelandic, and second sentece shows the mapping from Icelandic word to English word. What should be noticed is that the Icelandic word \textit{hversu} turns to two English words \textit{how much}, but actually \textit{hversu} only used as a degree operator, but not a manner adverbial, which makes question '\textit{how old are you?}' sounds like '\textit{old are you?}'.

\begin{enumerate}[resume]
\item \label{ref8}    
Hversu gammall ertu? \\ \textit{how.much old are.you} \\ 'How old are you'?
\end{enumerate}

To explain this problem, authers made a postulation of null operator in Null Degree Question. 
