...
By the perspective of gradable adjectives providing a "comparison" or "assignment", we can conclude the essence of superlative meaning and equative meaning as follows.

\begin{itemize}
    \item[1.] A comparative form expresses a superlative meaning if and only if this form is a individual set comparison.
    \item[2.] A positive form expresses a equative meaning if and only if this form has a explicit differential phrase. 
\end{itemize}

For example, under traditional point of view, \ref{positive_form_equative_meaning_example} is classified in positive form with positive meaning \hl{ref here}, but by this paper's approach, category should be positive form but equative meaning. Different form \ref{equative_meaning_example}, which assigns a value of individual's height to another individual's height, this example assigns a accurately value "\text{2 mi}" to another individual's height. When a specific value appears in positive form, this paper call it not differential phrase, but measure phrase(MP). Many researchers do not separate this two concepts very clear. Under out discussion, differential phrase appears in comparative form, measure phrase appears in positive form. 

\begin{enumerate}[resume]
    \item \label{positive_form_equative_meaning_example}
    John gao 2 mi. \\
    \textit{John tall 2 meters.} \\
    John is 2 metes tall.
\end{enumerate}
